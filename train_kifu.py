...
# Sample usage: python train_kifu.py dnnregressor 2000
...
if len(sys.argv) < 3:
  print('Usage: python train_kifu.py <dir> <steps>')
  exit(1)
...
training_set = tf.contrib.learn.datasets.base.load_csv_without_header(
    filename='test.csv', target_dtype=np.float32, features_dtype=np.float32, target_column=-1)
...
# Evalutes accuracy
results = estimator.evaluate(x=x_test, y=y_test, steps=1)
