...
def model_fn(features, targets, mode, params):
  ...
  # Infer special actions from move probilities.
  # TODO: add dense layer if required.
  special_actions = tf.layers.dense(inputs=conv_flat, units=2)
  pass_logit, other_logit = tf.split(special_actions, [1, 1], axis=1)

  # pass + 81 position + surrender
  logits = tf.concat([pass_logit, conv_flat, other_logit], 1)
