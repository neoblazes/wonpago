...
def model_fn(features, targets, mode, params):
  ...
  conv_out = tf.layers.conv2d(inputs=conv6, filters=1, kernel_size=[1, 1],
      padding="same") # To reduce size
  # Flattens conv2d output and use it as move probility.
  conv_flat = tf.reshape(conv_out, [-1, 9 * 9])
  ...
  #predict_qa = tf.reshape(tf.gather_nd(q_net,                   # [batch, 82]
